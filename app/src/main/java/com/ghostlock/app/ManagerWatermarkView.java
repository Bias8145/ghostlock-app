...
public final class ManagerWatermarkView extends View {
    ...
    private void drawShield(Canvas c, float s) {
        path.reset();
        path.moveTo(0, -s*.43f); path.cubicTo(s*.22f,-s*.36f,s*.35f,-s*.31f,s*.37f,-s*.26f);
        path.lineTo(s*.33f,s*.02f); path.cubicTo(s*.29f,s*.24f,s*.13f,s*.38f,0,s*.46f);
        path.cubicTo(-s*.13f,s*.38f,-s*.29f,s*.24f,-s*.33f,s*.02f); path.lineTo(-s*.37f,-s*.26f);
        path.cubicTo(-s*.35f,-s*.31f,-s*.22f,-s*.36f,0,-s*.43f); path.close(); c.drawPath(path,paint);
    }
