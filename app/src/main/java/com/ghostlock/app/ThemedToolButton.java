...
public class ThemedToolButton extends MaterialButton {
    ...
    private void showUrlDialog() {
        ...
        input.setSingleLine(true); input.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_URI); input.setHint("https://...");
        input.setTextColor(0xFFF0F1F3); input.setHintTextColor(0xFF8D9199);
        ...
        MaterialButton cancel = button("Cancel", false); cancel.setOnClickListener(v -> d.dismiss());
        MaterialButton parse = button("Parse", true); parse.setOnClickListener(v -> {
            String url=input.getText().toString().trim();
            if (!(url.startsWith("http://") || url.startsWith("https://"))) { input.setError("Enter a valid URL"); return; }
            d.dismiss(); invoke("runExtract", new Class[]{String.class, java.io.File.class}, new Object[]{url, null});
        });
        actions.addView(cancel, new LinearLayout.LayoutParams(-2,dp(48))); LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-2,dp(48)); p.leftMargin=dp(8); actions.addView(parse,p);
    ...
    private Dialog baseDialog(String title, String subtitle) {
        ...
        LinearLayout root=new LinearLayout(getContext()); root.setOrientation(LinearLayout.VERTICAL); root.setPadding(dp(22),dp(20),dp(22),dp(16)); root.setBackground(round(0xFF202124,26));
        TextView t=text(title,20,true); root.addView(t,margin(-1,-2,0,0,0,4)); TextView s=text(subtitle,12,false); root.addView(s,margin(-1,-2,0,0,0,12)); d.setContentView(root); return d;
    }
    private LinearLayout content(Dialog d){ return (LinearLayout)d.findViewById(android.R.id.content).getChildAt(0); }
    private void addAction(LinearLayout box,String title,String sub,Runnable r){ LinearLayout row=new LinearLayout(getContext());row.setOrientation(LinearLayout.VERTICAL);row.setPadding(dp(14),dp(9),dp(14),dp(9));row.setBackground(round(0xFF2A2D32,16));row.setClickable(true);row.setOnClickListener(v->r.run());TextView a=text(title,14,true);TextView b=text(sub,11,false);row.addView(a);row.addView(b);LinearLayout.LayoutParams p=margin(-1,dp(66),0,0,0,8);box.addView(row,p); }
    ...
    private TextView text(String s,int size,boolean bold){TextView v=new TextView(getContext());v.setText(s);v.setTextSize(size);v.setTextColor(bold?0xFFF1F2F4:0xFFB8BBC2);if(bold)v.setTypeface(null,android.graphics.Typeface.BOLD);return v;}
    ...
    private void size(Dialog d){if(d.getWindow()!=null){d.getWindow().setBackgroundDrawableResource(android.R.color.transparent);d.getWindow().addFlags(android.view.WindowManager.LayoutParams.FLAG_DIM_BEHIND);d.getWindow().setDimAmount(.68f);d.getWindow().setLayout((int)(getResources().getDisplayMetrics().widthPixels*.88f),-2);}}
    private void invoke(String name,Class<?>[] types,Object[] args){try{Method m=getContext().getClass().getDeclaredMethod(name,types);m.setAccessible(true);m.invoke(getContext(),args);}catch(Throwable t){Toast.makeText(getContext(),"Tool action failed: "+t.getMessage(),Toast.LENGTH_SHORT).show();}}
