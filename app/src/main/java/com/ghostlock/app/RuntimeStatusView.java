...
public class RuntimeStatusView extends LinearLayout {
    ...
    public RuntimeStatusView(Context context, android.util.AttributeSet attrs) {
        ...
        state = text(15, true); addView(state);
        message = text(12, false); addView(message, margin(-1, -2, 0, 5, 0, 0));
        kernel = text(11, true); addView(kernel, margin(-1, -2, 0, 10, 0, 0));
        manager = text(11, true); addView(manager, margin(-1, -2, 0, 4, 0, 0));
        action = text(12, true);
        action.setGravity(Gravity.CENTER); action.setPadding(dp(12), 0, dp(12), 0); action.setVisibility(View.GONE);
    ...
    public void refresh() {
        ...
        switch (result.state) {
            ...
            case MANAGER_REQUIRED:
                ...
                message.setText("Install a registered manager before running GhostLock"); setSurface(0xFF2C281E); showInstall = true; break;
            case KERNEL_UNSUPPORTED_MANAGER_REQUIRED:
                state.setText("KERNEL UNSUPPORTED"); state.setTextColor(0xFFFF7777);
                message.setText("The kernel is unsupported and no manager is installed"); setSurface(0xFF2B2022); break;
        ...
        kernel.setText("Kernel   ·   " + (result.kernelSupported ? "Supported" : "Unsupported"));
        kernel.setTextColor(result.kernelSupported ? 0xFF72D6A0 : 0xFFFF7777);
        String managerText;
        if (!result.manager.installed) managerText = "Manager  ·   Not installed";
        else if (result.manager.spoofed) managerText = "Manager  ·   " + result.manager.name + "  ·  Identity mismatch";
        else if (result.manager.identityVerified) managerText = "Manager  ·   " + result.manager.name + "  ·  Verified";
        else managerText = "Manager  ·   " + result.manager.name + "  ·  Recognized";
    ...
    private void showManagerPicker() {
        ...
        LinearLayout box = new LinearLayout(getContext()); box.setOrientation(VERTICAL); box.setPadding(dp(22), dp(20), dp(22), dp(16)); box.setBackground(round(0xFF202124, 26));
        TextView title = text(19, true); title.setText("Install supported manager"); box.addView(title);
        TextView subtitle = text(12, false); subtitle.setText("Select a registered manager to continue."); box.addView(subtitle, margin(-1, -2, 0, 4, 0, 12));
        for (ManagerCompatibility.ManagerInfo managerInfo : managers) {
            ...
            row.setGravity(Gravity.CENTER_VERTICAL); row.setPadding(dp(14), 0, dp(14), 0); row.setBackground(round(0xFF2A2D32, 16)); row.setClickable(true); row.setFocusable(true);
            row.setOnClickListener(v -> { dialog.dismiss(); ManagerCompatibility.openInstaller(getContext(), managerInfo); });
            box.addView(row, margin(-1, dp(54), 0, 0, 0, 8));
        }
        TextView cancel = text(13, true); cancel.setText("Cancel"); cancel.setGravity(Gravity.CENTER); cancel.setOnClickListener(v -> dialog.dismiss());
        box.addView(cancel, margin(-1, dp(44), 0, 2, 0, 0));
        dialog.setContentView(box);
        dialog.setOnShowListener(x -> { if (dialog.getWindow() != null) { dialog.getWindow().setBackgroundDrawableResource(android.R.color.transparent); dialog.getWindow().setDimAmount(.68f); dialog.getWindow().addFlags(android.view.WindowManager.LayoutParams.FLAG_DIM_BEHIND); dialog.getWindow().setLayout((int)(getResources().getDisplayMetrics().widthPixels*.88f), -2); }});
        dialog.show();
    }

    private TextView text(int size, boolean bold) { TextView view = new TextView(getContext()); view.setTextSize(size); view.setTextColor(0xFFE9EAED); if (bold) view.setTypeface(null, android.graphics.Typeface.BOLD); return view; }
