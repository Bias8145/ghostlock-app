...
public class RuntimeStatusView extends LinearLayout {
    ...
    private void installNavigation(){
        ...
        // Remove the redundant header actions. Theme and advanced controls now live in Settings.
        ...
        historyPage=buildHistoryPage(root);
        settingsPage=buildSettingsPage(root);
        pages.addView(historyPage,matchParent());
        pages.addView(settingsPage,matchParent());
        historyPage.setVisibility(GONE);
        settingsPage.setVisibility(GONE);
        rootGroup.addView(pages,index);

        configureNavButton(navHome,R.drawable.fa_house,"Home");
        configureNavButton(navHistory,R.drawable.fa_history,"History");
        configureNavButton(navSettings,R.drawable.fa_gear,"Settings");
        ...
        showPage(content,historyPage,settingsPage,0,navHome,navHistory,navSettings);
    ...
    private void hideHeaderTheme(View root){
        ...
        for(int i=0;i<group.getChildCount();i++){
            ...
            if(child instanceof ThemeToggleButton){child.setVisibility(GONE);continue;}
            if(child instanceof ViewGroup) hideHeaderTheme(child);
        }
    }
    ...
    private View buildHistoryPage(View root){
        ...
        MaterialButton copy=new MaterialButton(getContext(),null,com.google.android.material.R.attr.materialButtonOutlinedStyle);copy.setText("Copy");copy.setTextAllCaps(false);copy.setMinHeight(dp(40));copy.setIconResource(R.drawable.fa_history);copy.setIconSize(dp(16));header.addView(copy,new LinearLayout.LayoutParams(dp(100),dp(44)));
        card.addView(header);
        ...
        outer.addView(card,new LinearLayout.LayoutParams(-1,0,1));
        return outer;
    }
    ...
    private void showPage(View home,View history,View settings,int page,MaterialButton h,MaterialButton hi,MaterialButton s){
        home.setVisibility(page==0?VISIBLE:GONE);history.setVisibility(page==1?VISIBLE:GONE);settings.setVisibility(page==2?VISIBLE:GONE);
        tintNav(h,page==0);tintNav(hi,page==1);tintNav(s,page==2);
        if(page==1) updateHistoryLog();
    }

    private void updateHistoryLog(){
        if(historyLog==null) return;
        View root=findViewById(R.id.root);View source=root.findViewById(R.id.logView);
        if(source instanceof TextView) historyLog.setText(((TextView)source).getText());
        historyLog.postDelayed(this::updateHistoryLog,500);
    }

    private void configureNavButton(MaterialButton button,int icon,String label){
        button.setText(label);button.setTextAllCaps(false);button.setIconResource(icon);button.setIconSize(dp(20));button.setIconGravity(MaterialButton.ICON_GRAVITY_TEXT_TOP);button.setIconPadding(dp(4));button.setPadding(dp(8),dp(5),dp(8),dp(5));button.setContentDescription(label);button.setTextSize(11);}
    private void tintNav(MaterialButton button,boolean selected){
        int fg=ContextCompat.getColor(getContext(),selected?R.color.accent:R.color.text_secondary);button.setTextColor(fg);button.setIconTint(ColorStateList.valueOf(fg));button.setBackgroundTintList(ColorStateList.valueOf(ContextCompat.getColor(getContext(),selected?R.color.accent_container:R.color.surface_container_low)));}
