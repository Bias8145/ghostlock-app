...
public class RuntimeStatusView extends FrameLayout {
    ...
    public void refresh() {
        ...
        switch (result.state) {
            ...
            case KERNEL_UNSUPPORTED:
                state.setText("KERNEL UNSUPPORTED"); state.setTextColor(ContextCompat.getColor(getContext(), R.color.status_error)); message.setText("The current kernel is not supported by GhostLock"); setSurface(R.color.status_error_bg); break;
    ...
    private void showManagerPicker() {
        ...
        TextView title = text(20, true); title.setText("Install supported manager"); box.addView(title);
        TextView subtitle = text(13, false); subtitle.setText("Select a registered manager to continue."); subtitle.setLineSpacing(0f, 1.08f); box.addView(subtitle, margin(-1, -2, 0, 6, 0, 16));
        for (ManagerCompatibility.ManagerInfo info : managers) {
            TextView row = text(15, true); row.setText(info.name + (info.installed ? "  ·  Installed" : "  ·  Not installed")); row.setGravity(Gravity.CENTER_VERTICAL); row.setPadding(dp(18), 0, dp(18), 0); row.setMinHeight(dp(60)); row.setBackground(round(ContextCompat.getColor(getContext(), R.color.surface_container_low), 17)); row.setClickable(true); row.setFocusable(true); row.setOnClickListener(v -> { dialog.dismiss(); ManagerCompatibility.openInstaller(getContext(), info); }); box.addView(row, margin(-1, 60, 0, 0, 0, 10));
        }
        TextView cancel = text(14, true); cancel.setText("Cancel"); cancel.setGravity(Gravity.CENTER); cancel.setTextColor(ContextCompat.getColor(getContext(), R.color.accent)); cancel.setMinHeight(dp(48)); cancel.setOnClickListener(v -> dialog.dismiss()); box.addView(cancel, margin(-1, 48, 0, 5, 0, 0));
        ...
        Window window = dialog.getWindow(); if (window != null) { int width = Math.min((int) (getResources().getDisplayMetrics().widthPixels * 0.88f), dp(390)); window.setLayout(width, android.view.ViewGroup.LayoutParams.WRAP_CONTENT); }
    }
