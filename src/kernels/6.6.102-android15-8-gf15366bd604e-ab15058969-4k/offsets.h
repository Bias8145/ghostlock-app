OFFSETS_ENTRY(
    "6.6.102-android15-8-gf15366bd604e-ab15058969-4k",
    STRUCT_OFFSETS_6_6,
    .pselect_waiter_shift = -2,
    .off_init_task = 0x020FF340,
    .off_init_cred = 0x02111628,
    .off_root_task_group = 0x0230A400,
    .off_selinux_enforcing = 0x0234D530,
    .off_selinux_blob_sizes = 0x0166C488,
    .off_security_hook_heads = 0x0166BD70,
    .off_slide_nfulnl_logger = 0x02102258,
    .off_slide_boot_id = 0x02368A68,
    .off_slide_loggers_0_1 = 0x021021A8,
),

/* BTF reference (runtime uses target.h defaults): */
/* #define STRUCT_PAGE_SIZE 0x40 */
/* #define STRUCT_PAGE_COMPOUND_HEAD 0x8 */
/* #define STRUCT_PAGE_TYPE 0x30 */
/* #define STRUCT_SLAB_CACHE 0x8 */
/* #define STRUCT_MM_STRUCT 0x4C0 */
